...
// --- IMPORTANT ---
// (SOLVABLE_6_DIGIT_NUMBERS list remains the same for fallback)
...
const Gameplay = () => {
  ...
  const userId = "67f077fe6375079e25f2c22a"; // Replace with actual user ID if available
  const level = "1"; // Replace with actual level if available
  ...
  const fetchNewNumber = async () => {
    ...
    setFetchError(null);
    try {
      const response = await fetch('http://localhost:3000/game/level', {
        method: 'POST',
        headers: {
      ...
      if (data && data.numbers && Array.isArray(data.numbers)) {
        setNumber(data.numbers.join(''));
        setExpression(data.numbers.join(''));
      } else {
        console.warn("Invalid response format for numbers:", data);
        // Fallback to local generation if API data is invalid
      ...
      console.error("Error fetching new number:", error);
      setFetchError(error.message);
      setLoadingNumber(false);
      // Fallback to local generation on fetch error
      const randomIndex = Math.floor(Math.random() * SOLVABLE_6_DIGIT_NUMBERS.length);
      const newSolvableNumber = SOLVABLE_6_DIGIT_NUMBERS[randomIndex];
      setNumber(newSolvableNumber);
      setExpression(newSolvableNumber);
  ...
  useEffect(() => {
    fetchNewNumber(); // Fetch initial number on component mount
  ...
  const renderDigits = () => {
    if (loadingNumber) {
      return <p className="text-center text-gray-400">Loading number...</p>;
    }
    if (fetchError) {
      return <p className="text-center text-red-500">Error loading number: {fetchError}</p>;
    }
    if (!number) {
      return <p className="text-center text-gray-400">No number to display.</p>;
    }
